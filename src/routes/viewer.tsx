...
const COLORS = ["#FACC15", "#22C55E", "#EF4444", "#38BDF8", "#FFFFFF"];

export const Route = createFileRoute("/viewer")({
  head: () => ({
    meta: [
      { title: "Image Viewer · instructSite" },
      { name: "description", content: "Full-screen drawing viewer with mark-up tools." },
...
function ImageViewer() {
  ...
  const [name, setName] = useState<string>("image");
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("is-viewer-image");
    const storedName = sessionStorage.getItem("is-viewer-name");
    if (!stored) {
      navigate({ to: "/tooling" });
      return;
    }
