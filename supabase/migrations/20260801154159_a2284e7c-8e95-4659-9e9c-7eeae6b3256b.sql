ALTER TABLE public.diary_amendments
  ADD COLUMN IF NOT EXISTS previous_qs_verified_pct integer,
  ADD COLUMN IF NOT EXISTS new_qs_verified_pct integer;
...
AS $function$
  SELECT d.workface_id,
         LEAST(100, COALESCE(SUM(COALESCE(d.qs_verified_pct, d.manager_completion_pct, d.completion_pct)), 0))::numeric
...
AS $function$
  ...
  wf AS (
    SELECT w.zone_id,
           w.id AS unit_id,
           LEAST(100, COALESCE((
             SELECT SUM(COALESCE(d.qs_verified_pct, d.manager_completion_pct, d.completion_pct))
               FROM public.daily_site_diaries d
  ...
  loose AS (
    SELECT d.zone_id,
           ...
           LEAST(100, SUM(COALESCE(d.qs_verified_pct, d.manager_completion_pct, d.completion_pct)))::numeric AS pct,
